...
kv_list *kv_list_append(kv_list *kvl, char *k, char *v){
	if (kvl == NULL){
		...
		while (kvl->next != NULL) kvl = kvl->next;
...
char *str_slice(const char *str, int start, int end){
	...
	if (start > (len - 1)) return calloc(1, 1);
	if (start < 0) start = 0;
	if (end < 0) end = len - end;
	if (end > (len - 1)) end = len - 1;
	if (end < 0) return calloc(1, 1);
	if (start > end) return calloc(1, 1);
...
char *str_trim(const char *str, const char *strip){
	...
	while(start < len && strchr(strip, str[start]) != NULL) start ++;
	if (start >= len) return calloc(1, 1);
	while(end >= 0 && strchr(strip, str[end]) != NULL) end--;
	if (end < 0 ) return calloc(1, 1);
